...
public class CommonExceptionResolver extends SimpleMappingExceptionResolver {
	...
	protected ModelAndView doResolveException(HttpServletRequest request,
		...
		CommonException movieFinderException;
		...
		if (!(ex instanceof CommonException)) {
			...
			try{
				...
				movieFinderException = new CommonException("error." + className);
			} catch(Exception e){
				movieFinderException = new CommonException("error.common");
			}
					
		}else{
			movieFinderException = (CommonException) ex;
		}
